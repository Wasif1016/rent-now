...
import { CheckCircle2, Users, Gauge, Fuel, MapPin, Calendar, Phone, MessageCircle } from 'lucide-react'
...
export default async function ListingDetailPage({ params }: PageProps) {
  ...
  const vendorPhone = vehicle.vendor.phone || vehicle.vendor.whatsappPhone || process.env.NEXT_PUBLIC_DEFAULT_VENDOR_PHONE || '923001234567'
  const vendorWhatsApp = vehicle.vendor.whatsappPhone || vehicle.vendor.phone || process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || vendorPhone
          ...
          <div className="mt-4 md:mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-2 space-y-4 md:space-y-6">
                ...
                {otherImages.length > 0 && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 p-3 sm:p-4">
                    {otherImages.slice(0, 4).map((img, idx) => (
                      <div key={idx} className="relative aspect-video bg-zinc-100 rounded-md overflow-hidden">
                        <Image
                          src={img}
              ...
              <Card className="p-4 sm:p-6 border-0 shadow-md bg-white">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                  <div>
                    ...
                    {vehicle.vehicleModel && vehicle.title !== displayTitle && (
                      <p className="text-sm text-muted-foreground">{vehicle.title}</p>
                    )}
                  </div>
                ...
                {vehicle.description && (
                  <div className="mb-6">
                    <h2 className="text-lg font-semibold text-[#1a2332] mb-2">Description</h2>
                  ...
                  {capacity != null && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-[#f5f5f5]">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-[#1a2332]">{capacity} Passengers</p>
                      </div>
                    </div>
                  )}
                  {vehicle.year && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-[#f5f5f5]">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-[#1a2332]">{vehicle.year}</p>
                      </div>
                    </div>
                  )}
                  {vehicle.mileage != null && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-[#f5f5f5]">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-[#1a2332]">{vehicle.mileage.toLocaleString()} km</p>
                      </div>
                    </div>
                  )}
                  {vehicle.fuelType && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-[#f5f5f5]">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-[#1a2332]">{vehicle.fuelType}</p>
                      </div>
                    </div>
                  )}
                  {vehicle.transmission && (
                    <div className="flex items-center gap-2 p-2 rounded-lg bg-[#f5f5f5]">
                      <div>
                        ...
                        <p className="font-semibold text-[#1a2332]">{vehicle.transmission}</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {vehicle.features && Array.isArray(vehicle.features) && (vehicle.features as string[]).length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-base font-semibold text-[#1a2332] mb-2">Features</h3>
            ...
            <div className="lg:col-span-1">
              <Card className="p-4 sm:p-6 border-0 shadow-md bg-white sticky top-24">
                <h2 className="text-xl font-bold text-[#1a2332] mb-4">Quick Booking</h2>
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <p className="font-semibold text-[#1a2332]">{vehicle.vendor.name}</p>
                </div>
              </Card>
            </div>
          </div>
