...
import { getCityBySlug, getRoutesListGroupedByOrigin, getRoutesByCities, searchVehicles } from '@/lib/data'
...
export default async function RoutesPage({ searchParams }: PageProps) {
  ...
  const { routes: groupedRoutes, sortedCities: sortedCityNames } = await getRoutesListGroupedByOrigin()
  ...
  if (fromCitySlug && toCitySlug) {
    ...
    if (fromCity && toCity) {
      ...
      if (route && route.vehicles?.length) {
        ...
        const result = await searchVehicles({ citySlug: fromCitySlug, limit: 12 })
