...
export async function POST(
  ...
  try {
    ...
    if (!emailResult.success) {
      ...
      if (errorMessage.includes('Authentication failed') || errorMessage.includes('535')) {
        errorMessage = 'SMTP Authentication failed. Please check your BREVO_SMTP_USER and BREVO_SMTP_PASS in .env.local. Make sure you are using the SMTP password (not your account password) from Brevo dashboard. See BREVO_SMTP_SETUP.md for details.'
      }
    ...
    await prisma.vendor.update({
      where: { id },
      data: {
        registrationStatus: vendor.registrationStatus === 'ACCOUNT_CREATED' ? 'EMAIL_SENT' : 'EMAIL_SENT',
