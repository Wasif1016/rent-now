...
import { getAllCitiesWithCounts, getTownsByCity, getVehicleTypesWithVehicles, searchVehicles } from '@/lib/data'
...
export default async function SearchPage({ searchParams }: PageProps) {
  ...
  const cities = citiesData.map(c => ({ id: c.id, name: c.name, slug: c.slug }))
  ...
  const vehicles: Vehicle[] = vehiclesData.vehicles.map((v: any) => ({
    ...
    vehicleModel: v.vehicleModel ? { name: v.vehicleModel.name, vehicleBrand: v.vehicleModel.vehicleBrand } : null,
    vendor: {
      name: v.vendor.name,
