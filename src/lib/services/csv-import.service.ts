...
function validateRow(
  ...
  if (!data.email || !data.email.trim()) {
    return {
      valid: false,
      error: `Row ${rowIndex + 1}: email is required`,
    }
  }
...
export async function importBusinessesFromCSV(
  ...
  try {
    ...
    if (rows.length < 2) {
      return {
        ...result,
        errors: [{ row: 0, error: 'CSV must have at least a header row and one data row', data: {} as CSVRow }],
      }
    }
    ...
    // Validate headers
    const requiredHeaders = ['business_name', 'email', 'city']
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (emailsToCheck.has(data.email.toLowerCase())) {
        result.errors.push({
          row: i + 2,
    ...
    const businessesToInsert = businessesToCreate.filter((b) => {
      if (existingEmailSet.has(b.email.toLowerCase())) {
