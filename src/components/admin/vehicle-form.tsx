...
export function VehicleForm({
  ...
  const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = `admin-${user.id}/${fileName}.${fileExt}`;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!user) {
      setErrors({ general: "You must be logged in to update a vehicle" });
      return;
    }
    ...
    try {
      ...
      const url = vehicleId
        ...
        : "/api/admin/vehicles"; // Note: Creating vehicles not fully implemented here yet
      ...
      router.push(`/admin/vehicles?message=${message}`);
      router.refresh();
    } catch (error: any) {
    ...
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 mb-4">
          You must be logged in to update a vehicle.
        </p>
