...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate slug from title + capacity + timestamp
    const timestamp = Date.now().toString(36);
    const capacitySlug = seats ? `-${seats}-seater` : "";
    ...
    const slug = `${titleSlug}${capacitySlug}-${timestamp}`;
    ...
    const vehicle = await prisma.vehicle.create({
      data: {
        ...
        seatingCapacity:
          seatingCapacity != null ? Number(seatingCapacity) : null,
