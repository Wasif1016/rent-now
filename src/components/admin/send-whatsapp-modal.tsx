...
export function SendWhatsAppModal({
  ...
  const handlePreview = async () => {
    if (!session?.access_token) {
      setError('You must be logged in to preview')
      return
    }
    setLoading(true)
    setError(null)
  ...
  const handleOpenInWhatsApp = async (usePreviewMessage?: boolean) => {
    ...
    try {
      const message =
        usePreviewMessage && previewMessage.trim()
          ? previewMessage
          : await fetchPreview()
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Open WhatsApp with a pre-filled message for {businessName}. You
            will send the message from your own WhatsApp.
          </DialogDescription>
        </DialogHeader>
        ...
        {previewMode ? (
          <div className="space-y-4">
            <div className="p-4 bg-muted rounded-lg">
              ...
              <pre className="text-sm whitespace-pre-wrap font-sans p-3 bg-background rounded border border-border max-h-60 overflow-y-auto">
                {previewMessage}
              </pre>
            </div>
            {error && (
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={() => handleOpenInWhatsApp(true)}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="whatsapp-custom-message">Custom message (plain text)</Label>
              <Textarea
                id="whatsapp-custom-message"
                value={customMessage}
                onChange={(e) => setCustomMessage(e.target.value)}
                placeholder="Leave empty when using a template. Use {{business_name}}, {{email}}, {{password}}, {{login_url}}."
                rows={8}
                className="font-mono text-sm"
                disabled={!!(selectedTemplateId && selectedTemplateId !== 'custom')}
              />
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={() => handleOpenInWhatsApp(false)}
                disabled={loading || !hasPhone}
                variant="default"
              >
                Open in WhatsApp
