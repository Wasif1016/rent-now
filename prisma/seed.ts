...
async function main() {
  ...
  const cityNames = [
    'Abbottabad', 'Ahmedpur East', 'Arif Wala', 'Astore', 'Attock', 'Awaran', 'Badin', 'Bagh',
    'Bahawalnagar', 'Bahawalpur', 'Bajaur Agency', 'Bannu', 'Bat Khela', 'Battagram', 'Besham', 'Bhakkar',
    'Bhalwal', 'Buner', 'Burewala', 'Chakwal', 'Chaman', 'Charsadda', 'Chichawatni', 'Chiniot',
    'Chishtian', 'Dadu', 'Daska', 'Dera Ghazi Khan', 'Dera Ismail Khan', 'Dera Murad Jamali', 'Dipalpur',
    'Faisalabad', 'Farooqabad', 'Ferozwala', 'Ghotki', 'Gojra', 'Gujar Khan', 'Gujranwala', 'Gujrat',
    'Hafizabad', 'Hangu', 'Haroonabad', 'Hasilpur', 'Haveli Lakha', 'Hub', 'Hyderabad', 'Islamabad',
    'Jacobabad', 'Jalalpur Jattan', 'Jampur', 'Jaranwala', 'Jatoi', 'Jauharabad', 'Jhang', 'Jhelum',
    'Kabal', 'Kamalia', 'Kamber Ali Khan', 'Kamoke', 'Karachi', 'Karak', 'Kasur', 'Khairpur',
    'Khanewal', 'Khanpur', 'Kharian', 'Khushab', 'Khuzdar', 'Kohat', 'Kot Abdul Malik', 'Kot Addu',
    'Kot Radha Kishan', 'Kotri', 'Kulachi', 'Lahore', 'Lakki Marwat', 'Lala Musa', 'Larkana', 'Layyah',
    'Lodhran', 'Mailsi', 'Mandi Bahauddin', 'Mansehra', 'Mardan', 'Mian Channu', 'Mianwali', 'Mingora',
    'Mirpur', 'Mirpur Khas', 'Multan', 'Muridke', 'Muzaffarabad', 'Muzaffargarh', 'Narowal',
    'Nawabshah (Benazirabad)', 'Nowshera', 'Okara', 'Pakpattan', 'Panjgur', 'Pasrur', 'Pattoki', 'Peshawar',
    'Phool Nagar', 'Pishin', 'Quetta', 'Rahim Yar Khan', 'Rawalpindi', 'Renala Khurd', 'Sadiqabad', 'Sahiwal',
  ...
  const getCategoryId = (slug: string) => categoryRecords.find(c => c.slug === slug)?.id
  ...
  const legacyTypes = [
    { name: 'Cars', slug: 'cars', categorySlug: 'car', description: 'Sedan, Corolla, Civic and more' },
    { name: 'Hiace', slug: 'hiace', categorySlug: 'van', description: 'Toyota Hiace vans for group travel' },
    { name: 'Vans', slug: 'vans', categorySlug: 'van', description: 'Passenger and cargo vans' },
    { name: 'Coaster', slug: 'coaster', categorySlug: 'bus', description: 'Toyota Coaster buses' },
    { name: 'Buses', slug: 'buses', categorySlug: 'bus', description: 'Large buses for group travel' },
    { name: 'Wedding Cars', slug: 'wedding-cars', categorySlug: 'luxury', description: 'Luxury cars for weddings' },
  ]
  const vehicleTypeRecords: { id: string; slug: string; categoryId: string | null }[] = []
  for (const vt of legacyTypes) {
    const categoryId = getCategoryId(vt.categorySlug) ?? null
    const existing = await prisma.vehicleType.findFirst({ where: { slug: vt.slug } })
    let rec
    if (existing) {
  ...
  if (adminAuth?.user) {
    ...
    if (existing) {
      ...
      if (updateErr) {
        console.log(`  ⚠️  Admin role set for ${adminEmail}; password update failed: ${updateErr.message}`)
      } else {
        console.log(`  ✅ Admin role and password set for existing user: ${adminEmail} / ${adminPassword}`)
      }
    } else {
      console.log(`  ⚠️  Could not create admin (${adminError?.message || 'unknown'}); create ${adminEmail} in Supabase and set user_metadata.role = 'admin'`)
    }
  }
  ...
  // ============================================
  // 8. SEO DIMENSIONS & FAQ TEMPLATES
  // ============================================
  ...
  // Keyword dimensions for dynamic SEO URLs (rent-a-car, car-hire, rent-a-car-with-driver, etc.)
  const keywordConfig: Record<string, string> = {
    'car-on-rent': 'Car on rent',
    'car-hire': 'Car hire',
    'rent-a-car': 'Rent a car',
    'rent-a-vehicle': 'Rent a vehicle',
    'rent-a-car-with-driver': 'Rent a car with driver',
    'rent-a-car-without-driver': 'Rent a car without driver',
  ...
  for (const [slug, label] of Object.entries(keywordConfig)) {
    ...
    const titleCase = displayLabel.replace(/\w\S*/g, (t) => t.charAt(0).toUpperCase() + t.slice(1).toLowerCase())
    await prisma.seoDimension.upsert({
      where: {
      ...
      create: {
        slug,
        type: 'keyword',
        basePattern: `/${slug}/{city_slug}`,
        defaultH1Template: `${titleCase} in {city_name}`,
        defaultTitleTemplate: `${titleCase} in {city_name} | Rent Now`,
        defaultMetaDescriptionTemplate: `Looking for ${titleCase.toLowerCase()} in {city_name}? Compare verified rental vendors and book with a small advance.`,
        isIndexableByDefault: true,
      },
    })
  }
  console.log(`  ✅ ${Object.keys(keywordConfig).length} keyword SEO dimensions created`)
  ...
  const getRouteTypeId = (slug: string) => routeTypeRecords.find(r => r.slug === slug)?.id ?? routeTypeRecords[0].id
  ...
  console.log('  ✅ Removed all existing bookings, vehicle-route links, and vehicles')
  ...
  for (const route of routesForLinking) {
    const matchingVehicles = vehiclesForLinking.filter((v) => v.cityId === route.originCityId)
